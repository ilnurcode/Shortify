...
// Сохраняем access token в памяти через auth.setAccessToken
                    if (window.auth && typeof window.auth.setAccessToken === 'function') {
                        window.auth.setAccessToken(body.access_token);
                    } else {
// fallback: sessionStorage — если auth.js не подключён
                        sessionStorage.setItem('access_token', body.access_token);
                    }
                }
                info.style.color = 'green';
                info.textContent = body?.message || 'Регистрация успешна';
                form.reset();
// опционально: редирект на создание короткой ссылки
